...
1. **인터넷 연결 확인** - 최대 5회 재시도 (5초 간격)
2. **영어 명언 수집** - 바탕화면 파일에 직접 추가
3. **인터넷 연결 재확인**
4. **뉴스 크롤링** - 헤드라인 → 경제 → 사설 → 영문 주식 순차 실행
5. **바탕화면 바로가기 생성** - `C:\news` 폴더의 `.lnk` 파일
6. **실행 결과 요약** - 콘솔 출력 + 로그 파일 저장

각 크롤러는 독립적으로 실행되며, 하나가 실패해도 나머지는 계속 진행한다.

## 영문 주식 뉴스 상세

`run_eng_stock_check.py`는 finviz.com에서 최신 주식 뉴스들을 수집한다.
